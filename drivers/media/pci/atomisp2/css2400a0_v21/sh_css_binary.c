 ...
 */
	...
	ia_css_debug_dtrace(IA_CSS_DEBUG_TRACE,
		...
		if (!supports_bds_factor(candidate->supported_bds_factors, descr->required_bds_factor)) {
	ia_css_debug_dtrace(IA_CSS_DEBUG_TRACE,
		"sh_css_binary_find() [%d] continue: 0x%x & 0x%x)\n", __LINE__, candidate->supported_bds_factors, descr->required_bds_factor);
			continue;
		}
