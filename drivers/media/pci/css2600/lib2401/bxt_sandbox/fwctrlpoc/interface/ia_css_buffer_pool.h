...
struct ia_css_buffer_pool{
	...
	int (*init)(struct ia_css_buffer_pool *this_bufpool, unsigned int num_bufs);

	void (*uinit)(struct ia_css_buffer_pool *this_bufpool);
 ...
 /**
 * @brief ia_css_create_isys_bufpool() - Create an instance of isys buffer pool
 * @param buffer pool handle: number of buffer in buffers in the pool
 * @return  error code (errno.h)
 */
 extern void ia_css_create_isys_bufpool(
